...
While not shown above, feel free to use `<b>` and `<em>` in HTML5. `<b>` is meant to highlight words or phrases without conveying additional importance while `<em>` is mostly for voice, technical terms, etc.
