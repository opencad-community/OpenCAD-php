...
CREATE TABLE IF NOT EXISTS `ympq_citation_types`
(
...
CREATE TABLE IF NOT EXISTS `ympq_incident_types`
(
    `id` INT(11) AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `ympq_radio_codes`
(
    `id` INT(11) AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `ympq_warning_types`
(
    `id` INT(11)  AUTO_INCREMENT,
...
CREATE TABLE
IF NOT EXISTS `ympq_warrant_types`
(
    `id` INT
(11)  AUTO_INCREMENT,
    `warrant_description` VARCHAR
(255)  COMMENT 'Description of a frequently used warrant type.',
    PRIMARY KEY
(`id`)
) ENGINE = InnoDB DEFAULT CHARSET = latin1;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `dl_type`
set('Not Issued','Learners','Provisional','Open','Identification Only')  DEFAULT 'Not Issued' AFTER `dl_status`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `dl_class`
set('Car','Light Rig','Heavy Rig','Boat','Motorbike','Military')  AFTER `dl_type`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `dl_issuer`
set('Government','Military')  AFTER `dl_class`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `weapon_permit`
set('Unobtained','Vaild','Suspended','Expired','Canceled')  AFTER `build`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `weapon_permit_type`
...
ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `weapon_permit_Issued_by`
set('Ammu-Nation','Government','Military','Unknown') DEFAULT 'Unknown' AFTER `weapon_permit_type`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `deceased`
set('NO','YES')  AFTER `organ_donor`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `race`
set('Indian','Asian','Black or African American','Hispanic','Caucasian','Pacific Islander','Unknown') DEFAULT 'Unknown' AFTER `gender`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `blood_type`
set('A+','O+','B+','AB+','A-','O-','B-','AB-','Unknown') DEFAULT 'Unknown' AFTER `weapon_permit_issued_by`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `organ_donor`
set('NO','YES')  DEFAULT 'NO' AFTER `blood_type`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `build`
set('Average','Fit','Muscular','Overweight','Skinny','Thin') AFTER `hair_color`;

ALTER TABLE `ympq_ncic_names` MODIFY IF EXISTS `hair_color`
...
ALTER TABLE `ympq_ncic_plates` MODIFY IF EXISTS `veh_insurance`
set('VALID','EXPIRED','CANCELED','SUSPENDED','Unknown') DEFAULT 'Unknown' AFTER `veh_scolor`;

ALTER TABLE `ympq_ncic_plates` MODIFY IF EXISTS `veh_insurance type`
set('CTP','Third Party','Comprehensive','Unknown') DEFAULT 'Unknown' AFTER `veh_insurance`;

ALTER TABLE `ympq_ncic_plates` MODIFY IF EXISTS `flags`
...
ALTER TABLE `ympq_ncic_plates` MODIFY IF EXISTS `veh_reg_state`
