...
CREATE TABLE IF NOT EXISTS `<db_prefix>ncic_names` (
  ...
  `dl_type` set('Learners','Provisional','Open') NOT NULL DEFAULT 'NONE',
...
CREATE TABLE IF NOT EXISTS `<db_prefix>ncic_plates` (
  ...
  `flags` set('STOLEN','WANTED','SUSPENDED REGISTRATION','CANCELED REGISTRATION','EXPIRED REGISTRATION','INSURANCE FLAG','DL FLAG','UC FLAG','HPIU FLAG') NOT NULL DEFAULT 'NONE',
